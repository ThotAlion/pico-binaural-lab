...
int magBuffer[SAMPLES];    // ApproxFFT sample buffer
int phaseBuffer[SAMPLES];  // ApproxFFT sample buffer
dma_channel_config cfg;
int dma_chan;

absolute_time_t t0, t1, t2, t3;
...
uint8_t RSSdata[20] = {7, 6, 6, 5, 5, 5, 4, 4, 4, 4, 3, 3, 3, 3, 3, 3, 3, 2, 2, 2};
...
{
    int temp3, m1, m2;
    uint8_t temp1, temp2, test, quad, accuracy;
...
//--------------------------------Fast ARG---------------------------------------//
double fastARG(int a, int b)
{
    // Approximation de l'argument en radians
    if (a == 0)
    {
        return (b > 0 ? 90 : (b < 0 ? -90 : 0)); // Cas particulier lorsque a est 0
    }

    double approx_radians = atan2(fastAbs(a), fastAbs(b));

    // Conversion en degrés
    double approx_degrees = approx_radians * RAD_TO_DEG;

    // Ajuster en fonction du quadrant
    if (a < 0)
    {
        if (b >= 0)
        {
            approx_degrees = 180 - approx_degrees; // Quadrant 2
        }
        else
        {
            approx_degrees = -180 + approx_degrees; // Quadrant 3
        }
    }
    else
    {
        if (b < 0)
        {
            approx_degrees = -approx_degrees; // Quadrant 4
        }
    }

    return (int)approx_degrees;
}

float Approx_FFT(int input[], unsigned int N, float Frequency, int mag[], int phase[])
{
    int a, c1, f, o = 0, x, data_max, data_min = 0;
    long data_avg, data_mag, temp11;
    uint8_t scale, check = 0;
    ...
    */
    ...
    {
        mag[i] = fastRSS(out_r[i], out_im[i]);
        // Approx RSS function used to calculated magnitude quickly
        phase[i] = fastARG(out_r[i], out_im[i]);
    }
...
int main()
{
    ...
    while (true)
    {
        ...
        {
            approxBuffer[i] = sampleBuffer[i] - 2048;
        }
        ...
        //////////////////////////////////////////
        // compute fft
        Approx_FFT(approxBuffer, SAMPLES, SAMPLING_FREQUENCY, magBuffer, phaseBuffer);
        ...
        for (int i = 0; i < SAMPLES / 2; i++)
        {
            printf("%d\t", phaseBuffer[i]);
        }
        printf("\n");
        // printf("t1-t0:%dus, t3-t2:%dus, fmax:%f\n",absolute_time_diff_us(t0,t1),absolute_time_diff_us(t2,t3));
